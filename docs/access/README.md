...
| [backup-restore.md](backup-restore.md) | The estate's backup & restore story: what's protected across all four repos, the manual `backup.yml` workflow, D1 Time Travel + export/import, the Firestore dump/restore scripts, R2's real gaps, and what's deliberately not backed up and why. ⚠️ Its §4b "just `wrangler d1 execute --file`" recipe is **wrong for two of the four databases** — see RECOVERY.md §3b |
| [RECOVERY.md](RECOVERY.md) | ⚠️ **The 3am runbook** — the *do this now* companion to backup-restore.md, every command executed in a sandbox on **2026-08-17/18** with production READ-only. Per-store restore recipes with measured times, and the four findings that only a real drill surfaces: the `library-catalog` and `board-game-catalog` **exports do not replay as-is** (`no such table`, dies half-imported — `scripts/reorder-d1-dump.mjs` is the fix); the Firestore restore **writes every one of 2,139 timestamps back as a map**; restoring `estate_auth` blind **silently re-approves a revoked member**; and the live stores with no backup at all (`library-catalog-2nd`, `discord_links`, `readingPositions`, four R2 buckets, one KV) |
