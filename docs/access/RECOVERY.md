...
| `INDEX_PUSH_TOKEN_LIBRARY` / `_AUDIOBOOK` / `_GAME` **(added 2026-08-26, secrets review)** | ⚠️ **PAIRED, per source** — `catalog-index` (verifier) + library main / audiobook `.env` / BGC, each as un-suffixed `INDEX_PUSH_TOKEN` | library + BGC `.dev.vars`; ✅ `audiobook_catalog/.env` | self-generated, **one value per source**. ⚠️ The index derives the SOURCE from which suffixed secret matched — a shared value files rows under the wrong catalog. ⚠️ ~~`INDEX_PUSH_TOKEN_LIBRARY2` is deliberately absent on both sides (padhard does not push)~~ — **SUPERSEDED 2026-09-05, see the row below: padhard pushes now** |
| `INDEX_PUSH_TOKEN_LIBRARY2` **(added 2026-09-05, federation day)** | ⚠️ **PAIRED** — `catalog-index` (verifier, suffixed name) + library **`--env friend`** / `library-catalog-friend` (padhard, as un-suffixed `INDEX_PUSH_TOKEN`) | 🔴 **NONE** — no `.dev.vars.friend` by design, and 1Password is deferred. Recovery is **re-mint the pair**, not retrieval | self-generated (`randomBytes(32).base64url`), minted 2026-09-05 16:03 Phoenix. ⚠️ **A DIFFERENT VALUE from `INDEX_PUSH_TOKEN_LIBRARY`** — the index derives the source from which suffixed secret matched, so a shared value would file padhard's rows under `library` and the snapshot-replace would delete the main library's shelf (`docs/info/index-worker-design.md` §11.2). ⚠️ Also different from `INDEX_READ_TOKEN_LIBRARY2` — read and push are never the same credential. Re-mint: pipe one value into `wrangler secret put INDEX_PUSH_TOKEN_LIBRARY2` (index-worker) **and** `wrangler secret put --config apps/worker/wrangler.toml --env friend INDEX_PUSH_TOKEN` (library repo); `scripts/push-secrets.mjs` refuses to copy main's, on purpose. Verify: `GET index.heygabi.ai/api/health` → `sources.library2.rows > 0` (unset ⇒ worded 503 `push_token_unset`) |
...
| **`catalog-provisioning.private.jwk`** — the sealed-catalog-key PRIVATE half **(added 2026-09-05)** | Nothing runs it as a service. Its ONE consumer is `scripts/lib/catalog-seal.mjs`, run by hand from a dev machine during a provision | 🔴 **`catalog-platform/docs/access/keys/catalog-provisioning.private.jwk` on THIS MACHINE, and nowhere else.** ✅ **It IS covered by the nightly docs backup** — MEASURED 2026-09-05: `scripts/backup-docs.mjs` applies no extension filter and no denylist (`:108–117`, skipping only `node_modules`/`.git`/`__pycache__`), and its dry-run inventoried **82 files, exactly the 82 on disk**, with `docs/access/keys/` holding 5. So it lands in `estate-backups/docs/catalog-platform/<UTC>.json.gz` at the 3am `EstateDocsBackupR2` task (`DONE.md` K4). ⚠️ That is the SAME machine's scheduled job writing to R2 — it is a second copy, not a second custodian. ✅ **A copy is in 1Password `Estate` since 2026-09-05 16:15Z** — DOCUMENT item `catalog-provisioning.private.jwk` (id `wjsfpbl4hw3zujxdctiaoonvyu`, tags `estate, catalog-provisioning, request-a-catalog`), written by `op document create <file> --vault Estate` so the value never passed through a session; verified by name and size only (attached file 3,333 B = the file on disk). Restore: `op document get catalog-provisioning.private.jwk --vault Estate --out-file docs/access/keys/catalog-provisioning.private.jwk` | 🔴 **NOT RE-MINTABLE AS A RECOVERY.** `node scripts/catalog-key-mint.mjs --rotate` makes a NEW keypair, and every envelope already in `estate-catalog-keys` becomes permanently undecryptable — the plaintext exists nowhere else by design (§6.5). Losing this file therefore costs *"ask every pending requester for their key again"*, not *"restore a secret"*. The script refuses to overwrite, and `--rotate` refuses again without `--i-have-checked-pending-envelopes` |
| ⚠️ **`scripts/lib/backup-keys.mjs` is NOT about this folder** (noted 2026-09-05, because the name reads like it is) | — | It is the **backup GENERATION-key grammar** — `<kind>/<store>/<UTC-STAMP><suffix>`, and how a split multi-part dump still counts as one generation for retention. Nothing in it touches `docs/access/keys/`. The file that backs that folder up is `scripts/backup-docs.mjs` | — |
