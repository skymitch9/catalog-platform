...
- [ ] ❓ **17 · (added 2026-09-07 02:52 Phoenix) Should `discord-worker` (GABI)
      also become a `deploy.yml` target?** The audiobook-worker target landed
      (`acf5f59`, first CI deploy `d4e74421` — see `DONE.md`). GABI is the same
      shape as auth-worker (it HAS D1, so the migrate step stays). Rec: yes —
      one target per Worker, so no deploy is ever hand-run from a phone again.
      Deliberately NOT built without the answer.
...
- [ ] 🧑 **9 · GABI steps 5–7 — he asked for the test, the test was given,
      ...
      out in full in the `GABI_CLUB_WRITES` section further down this file:
      re-run `POST /admin/commands/register` with an admin Firebase ID token
      (nothing is visible in Discord until that happens), opt one club in
      (`features.meetingRsvp = true`), then run `/rsvp` and `/progress` and
      ⚠️ **look at the club PAGE** — the Discord reply is not the evidence.

**⏸ Deferred by the owner in his own words**, kept visible so they are not
re-derived, each with the file that owns the detail: **8** Diva's items
(#8 / #9 / #605) — *"Ignore for now"*; **10** exporting
`ESTATE_APP_TOKEN_LIBRARY` / `_LIBRARY2` into the CLI's shell — *"Okay we do
later"* (it belongs to the billing soak, item 5 of the LLM-billing section
...
## ☐ Federation leftovers (2026-09-06 08:46 Phoenix) — ④ the owner's eyeball, two ❓ decisions, and what nobody has exercised

> The multi-library ask of 2026-09-05 15:50 moved WHOLE to [`DONE.md`](DONE.md)
> at 08:46 — all four of survey §10's dispatches landed and the ownership table
> is settled. These are the things it left open, each with the evidence already
> recorded. As-built:
> [`info/catalog-registry.md`](info/catalog-registry.md) §10 and §10a ·
> [`info/request-a-catalog-design.md`](info/request-a-catalog-design.md) §7.6b ·
> `library_catalog/docs/info/peer-network.md`.

- [ ] 🧑 **④ THE OWNER'S EYEBALL — item 14 of the sixteen, and the last step
      of the whole ask.** He asked for a task to check it with; this is it.
      Signed in, open <https://heygabi.ai/universes/> and
      <https://heygabi.ai/series/> and confirm **Padhard's rows appear in both
      tabs**, then look at the index panel on <https://heygabi.ai/status/> for
...
- [ ] 🧑 **The audiobook re-vendor is on the `/dev/` lane only — its prod
      ...
      (`scripts/sync_estate_search.py` + a read-only drift test) resolved all
      four of survey §5's divergences, two of them user-visible: clicking a
      cover did nothing, and a second household's shelf could not be named.
      ⚠️ The commit is **not book-only**, so `auto-promote.yml` will not carry
      it.
- [ ] **A measured push cadence for `library2` on `/status`.** The index panel
      grades only the three sources somebody actually measured; a catalog that
      joined afterwards has its age SHOWN and deliberately NOT graded, because
