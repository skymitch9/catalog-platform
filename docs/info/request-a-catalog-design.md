...
✅ **BUILT 2026-09-05 — `library_catalog` commit `b84b39f`** (runbook: that repo's `docs/access/provision-catalog.md`); ⚠️ **unexercised — nothing has run past `--dry`, so no instance exists because of it.**
...
| **3b** | ✅ **BUILT `1bfb5ac`** — the "+" bottom-right on **both** cards, the modal, the required review step, the pending pill, fail-hidden, per-kind show/hide. New `sites/heygabi-home/public/assets/apex-request-catalog.js`; styles in `index.html`'s own `<style>` (the `apex-admin-link.js` precedent — that module has no stylesheet either); one `data-catalog-kind` hook per card. ✅ **DEPLOYED `4875d1dd`** from a worktree of `597e40d`, after the CORS block below cleared | same | ~1 day | **VERIFIED LIVE** (`curl -s -D -`, cache-busted): `/` 200 serving both `data-catalog-kind` hooks, the Games host **link**, and the script tag; `/assets/apex-request-catalog.js` 200, 27,939 bytes, all six pinned strings; ⚠️ **no `card-add` and no `card-pending` anywhere in the served HTML** — the signed-out page carries the hooks and nothing else, which is the gate working; `verify:home` 29/29. **EXERCISED BEFORE SHIPPING, not reasoned about:** a stub-DOM harness drove the real module through all nine §4.3 rows (signed out · probe 500 · not approved · **`catalogs` field absent** · approved/none · books-live · games-pending · books-accepted · probe-throws) — nine of nine correct; the review gate refusing an empty and a malformed address **in words with zero fetches**; the POST body exactly `{kind, desired_subdomain, display_name, extra:{note}}`; a 409 rendering the route's own sentence verbatim; a thrown fetch rendering the **outage** sentence; the availability debounce firing 0 fetches mid-keystroke and one afterwards with the reserved wording; withdraw arming, POSTing `…/requests/42/withdraw`, restoring the "+". `check:home` green (30 JS · 26 graphs · 14 HTML). ⚠️ **STILL THE BAR: A HUMAN, SIGNED IN, PRESSES BOTH** at <https://heygabi.ai> and files one real request of each kind. Nothing here has ever spoken to a real route |
...
| **6** | ✅ **DONE 2026-09-05 07:31 Phoenix** — back-seeded by hand (main loop, `wrangler d1 execute estate_auth --remote`) as rows **#1** `library` (books, `nbaslamking@gmail.com`, instance `main`, host `library.heygabi.ai`), **#2** `padhard` (books, Samantha Hardman, instance `friend`, host `padhard.heygabi.ai`), **#3** `boardgames` (games, the owner, instance `main`, host `boardgames.heygabi.ai`); all `status='live'`, `decided_by=1`, `owner_key_set=1`, `extra={"backseed":"2026-09-05","estate_app":…,"repo":…}`. Read back with `SELECT … ORDER BY id`: 3 rows, `changes:3`. ⚠️ Consequence: the owner's primary account now sees NO "+" on either card — to test the "+" he signs in as his second approved account (`estate_user` #87), which owns nothing | D1 data | done | **NOT yet verified in a browser**: each owner's "+" hidden **for the right kind only**, signed in as each |
| **7** | ✅ **BUILT 2026-09-05, `library_catalog` `b84b39f`** — `scripts/provision-catalog.mjs` (§7.4), the BOOKS path; **v1 sets the OWNER's `ANTHROPIC_API_KEY`** (§6.4 row 3, owner decision 2026-09-05). ⚠️ **Half-verified:** `--dry` prints all twelve steps and both pauses (68 unit tests, suite 2418/0), but **no real instance exists** — nothing has run past `--dry`, and the first real run is the owner's | `library_catalog` | ~2 days | `--dry` prints all ten steps; a **real third instance answers `/api/health?cb=`** and its first sign-in logs `src:"seen"` under the new app id — ⚠️ **the second half is still owed** |
| **8** | ✅ **LANDED 2026-09-05** — §8 items 1–3 in `Board_Game_Catalog`: `fc17ea3` `ESTATE_APP` lifted out of source (`src/lib/estate-app.ts`; a typo turns the gate OFF rather than falling back to `games`) with the same-id build guard; `30dc045` the `:games2` script twins + a `push-secrets --env` that refuses `ANTHROPIC_API_KEY` and every `ESTATE_APP_TOKEN_*`; `4db2f2e` a commented `[env.<instance>]` TEMPLATE with a drift guard. Docs `5ff223a`/`9c1dba6`, deploy `a812252`. ⚠️ The deploy guards were **already** instance-aware — that half of item 1 was stale when written | `Board_Game_Catalog` | §8 — landed in four commits, main-instance behaviour unchanged | ✅ **The guard was SEEN TO REFUSE**, twice, by being made to fail: a duplicate `[env.games2.vars] ESTATE_APP = "games"` appended to the real `wrangler.toml` → *"two wrangler environments both declare ESTATE_APP = "games" ([vars] and [env.games2.vars]) … This is estate credentials catalog F-5"*; and a var added to `[vars]` but not the template → the template-drift refusal. Both reverted; the refusing cases are also permanent fixture tests (suite 189 → 220). ✅ Main instance redeployed `9c1dba6f` → `a349aee1`, `/api/health?cb=` **200** with a new `estate` block reading `app:"games"`, `tokenVar:"ESTATE_APP_TOKEN_GAMES"`, `configured:true`. 🔴 `RATE_LIMITER` **MEASURED**: `namespace_id` is scoped **per ACCOUNT** — Cloudflare's docs, *"even across different Workers on the same account — share the same rate limit counters"* — so a second instance declares its own (`"1002"`); `Board_Game_Catalog/docs/info/instance-model.md` §3. ⚠️ **NOT verified:** no second instance exists (no D1, R2, hostname or deploy) and nobody signed in — `configured:true` proves both halves EXIST, not that the directory accepts the token |
| **9** | ✅ **BUILT 2026-09-05, `Board_Game_Catalog`** — `7b6b049` closes phase 8's open item (`BILLING_SITE` → `billingSite(env)`, following `ESTATE_APP`, because `siteForApp()` proves the site id and the app id are one identity; an unrecognised id gives `null`, never `games`; **not deployed and provably a no-op** — `BILLING_POLICY="off"` means neither call site runs, and `games` returns `games`). `b11a373` `scripts/provision-catalog.mjs`, the §7.6 ledger; `ad2258c` 74 tests (suite 220 → **298/0**); `54faef0` the runbook `docs/access/provision-catalog.md`. ⚠️ **Half-verified, like phase 7:** nothing has run past `--dry` and no second instance exists | `Board_Game_Catalog` | after 8 | ✅ `--dry` prints all twelve steps, both pauses and a 109-line rendered block (exit 0), with the **live** Firebase authorised-domain list read; and it refuses **against the real D1** — row #3 `boardgames` "already live" and row #1 `library` "this is a BOOKS request", **exit 2** both, so the read path, the column mapping and both refusals are exercised on production data. ✅ `RATE_LIMITER` was measured at phase 8 and the picker **never returns `"1001"`** (a test pins it). 🔴 **Four defects were found by RUNNING it:** a key-only TOML substitution rewrote `name = "RATE_LIMITER"` inside the unsafe binding to the Worker's name (TOML reuses short keys across tables, so a key is not an address); `Number(null)` is `0`, so an absent `--request` read as request #0; `--resume` threw a refusal about `games3` when an id was already pinned; and the secret plan's last-moment guard was unreachable because it re-used the classifier the loop had filtered with. ⚠️ **NOT verified:** no D1, bucket, covers hostname, secret or deploy exists; no envelope was decrypted from this side (the seal lib was exercised through stubs, one per `source`); nobody signed in. 🔴 **NEW, and it belongs to `catalog-platform`:** adding `games2` to `CONSUMER_APPS` also needs a `case 'games2'` in `siteForApp()` (`apps/auth-worker/src/estate.ts:118`) and `'games2'` in `BILLING_SITES` (`billing-registry.ts:38`) or **that repo does not compile** — `siteForApp` is exhaustive over `ConsumerApp`. §7.6's row 5 and §7.2's step 5 both understate the manual step by two lines; the provisioner's PAUSE #2 prints all of it. ☐ **Owner decision still open:** the naming split (a)/(b)/(c) — both provisioners are built to (a) so the pair agrees, each behind ONE function |
| **5 → LAST** | The sealed key: keypair, the browser seal, the private-R2 envelope, the booleans — **deliberately the final phase** (owner, 2026-09-05: *"Defer it until everything else is built then build it … the defer is until after the other bits build but not forever"*). The number is kept so §6's cross-references stay valid.<br><br>• **SERVER half — ✅ LANDED + DEPLOYED 2026-09-05 (agent S1)**, `estate-auth` **`d87235f8-c2a1-4756-bacf-ac2a23da880e`** at commit `9e0922f`. Commits: `f3b40b6` the private bucket **`estate-catalog-keys`** (created 14:47:18Z — it did **not** already exist; `dev-url get` → *"Public access via the r2.dev URL is disabled"*) + the `CATALOG_KEYS` binding + the `Env` field; `e722d01` `src/catalog-keys.ts` (the contract + the store) and the four route changes in `src/catalog-requests.ts`; `e8254db` 22 tests; `9e0922f` the `/me` bare-status fix that rode along.<br>• **CLIENT + PROVISIONER half — ✅ BUILT 2026-09-05 (agent S2). 🔴 NOT DEPLOYED — the conductor's single home deploy carries it, per the one-deployer-per-Pages-project rule.** `catalog-platform`: `c53d361` the mint script + `assets/catalog-seal.js` — **the real provisioning keypair was minted, kid `fb6eb908ead63ce7`**, private JWK at `docs/access/keys/catalog-provisioning.private.jwk` (`git check-ignore` confirms `.gitignore:67` excludes it and `git status` never saw it); `342d7be` `scripts/lib/catalog-seal.mjs` + 34 tests; `d687264` the "+" form's optional field, a **committed** stub-DOM harness (`scripts/test/helpers/stub-dom.mjs` — phase 3b built one and threw it away) + 10 scenarios; `6cbb452` the Accept panel's §5.4 items 3–4 + the `predeploy.checks.json` pins; `e63ccbe` the absent-envelope fix below. `library_catalog`: `acb4c44` step 10's §6.4 ladder + `markLiveUpdate()` + the runbook.<br><br>⚠️ **The client BELIEVES THE ROW, NOT ITS OWN POST** — both surfaces read `reader_key_set`/`owner_key_set` back and say in words when it is not `1`, because an older Worker ignores an unknown body field and answers a cheerful 201/200. ⚠️ **The "+" form seals at the REVIEW step, not at submit**, so the plaintext lives for one handler and only the envelope crosses into the draft. ⚠️ **A refused seal ABORTS the /admin Accept** but never the requester's submit: a requester can be asked for a key again, an owner who thinks he set one finds out on a bill.<br><br>⚠️ **`/live` deliberately deletes nothing** — the provisioner deletes each object as it injects it, because that is the only place that knows the `secret put` landed; `{"purge_keys": true}` is the hatch for a run without the library. ⚠️ **Decline and withdraw delete BOTH objects and clear BOTH booleans**, because before provisioning a boolean means *"an envelope is held"*; at `/live` the same two columns are the provisioner's statement about the INSTANCE, which is why phase 6's back-seeded rows carry `owner_key_set = 1` with no envelope | both halves | ~1 day | A round trip: seal in a browser, decrypt in a script, confirm the decrypted bytes equal the input — **and confirm no code path prints it**.<br><br>**SERVER half, as built.** **MEASURED:** migrate-before-deploy ran in that order and answered **"No migrations to apply!"** — `reader_key_set`/`owner_key_set` have existed since 0018, so this phase needed **no schema change**, and that is a measurement rather than an assumption. Suite **651 → 674 pass / 0 fail**, `tsc` clean on both projects, `cors-coverage.test.ts` green (this phase adds **no new PATHS**, only fields on three existing ones). **VERIFIED LIVE** (`curl -s -D - … -o /dev/null` — ⚠️ `-I` and `-o NUL` misreport on this host): `/api/health` **200**; an unauthenticated `POST …/requests` **carrying a `sealed_key`** → **401** with the worded refusal, so auth still gates before the envelope is ever looked at; `OPTIONS` preflight from `https://heygabi.ai` → **204** on the bare mount **and** the wildcard; `GET /api/estate/me` signed out → **401 with the new `detail`**. Three properties are pinned mechanically rather than argued: **no route can return an envelope** (the collection read, as approver and as requester, is searched for the ciphertext, the wrapped key and the literal field names, and `/me`'s projection likewise); **nothing is logged** (every `console` method captured across every key-touching route *including the failure paths*, then searched for `ct`/`ek`/`iv`/`kid`); and **the Worker never READS the bucket** (the stub implements `get`/`list` only so tests can look inside, counts every such call, and the count must be **0** — the mechanical form of §6.2's *"the absence of a decrypt-to-read path, not a policy, is the guarantee"*). 🔴 **NOT VERIFIED: no signed-in request with a real envelope has ever been filed.** A session cannot sign in as a person, so every 200-side path is proven **only** against an in-memory D1 plus an R2 stub — the owner's browser test with S2's form is the live proof, and nothing here substitutes for it.<br><br>**CLIENT + PROVISIONER half, as built.** **MEASURED:** the round trip is real and Node-to-Node — the **actual browser module** seals and `scripts/lib/catalog-seal.mjs` opens, bytes equal, with a THROWAWAY keypair minted in the test; a maximal 512-byte plaintext serializes to ~1.5 KB against the 8 KB ceiling; `ek` is exactly 512 bytes, `iv` 12, `ct` plaintext+16; a second keypair's envelope is refused **by kid, in words about rotation**, not as a crypto error. The "+" form was **exercised, not reasoned about**, through a stub DOM: field hidden AND silent when `sealSupported()` is false (and submit still works) · an empty field puts **no** `sealed_key` on the wire, absent rather than null · a filled field sends the six-field envelope while a CANARY plaintext appears in **none** of the request body, the dialog, the page body or the console · an over-long key refused in words with **zero** POSTs · `reader_key_set` absent, `0`, and a server `warnings[]` each rendered as the warning · Back offering to replace or remove a key it cannot show. `check:home` 31 JS parsed / 28 module graphs resolved / 14 HTML. Suites: platform scripts **277 → 324**, `library_catalog` **2418 → 2428**, both 0 fail.<br><br>🔴 **THE MEASUREMENT THAT PAID FOR ITSELF, and it was found in a REHEARSAL:** the provisioner's `--dry` reported **both** envelopes PRESENT against a bucket holding neither. `defaultRunner().get()` had inherited the Windows teardown tolerance (*"a usable body plus a non-zero exit is a success"*, `library_catalog/scripts/lib/d1.mjs`) as *"any stdout means a hit"* — and wrangler **4.123.0** answers a MISSING object with **exit 127**, `The specified key does not exist.` on stderr **and a single newline on stdout**. Fixed (`e63ccbe`): stderr first, body must be non-whitespace, a missing BUCKET distinguished from a missing OBJECT so *"not deployed yet"* is never read as *"no key attached"*; both real stderr strings and four failures that must NOT read as absent are now fixtures.<br><br>🔴 **NOT VERIFIED, and this is the bar:** **no human has sealed a key in a real browser.** Nothing on the home site is deployed by S2. No envelope has ever been written to `estate-catalog-keys` or decrypted out of it — rows 1 and 2 of §6.4's ladder are proven only Node-to-Node with a throwaway key. Nobody has rendered the Accept panel signed in; there is still no browser harness for `admin.js`. And `--pipe` + `--remote` are verified against `--help` and a live *absent* object, never against a real envelope |
