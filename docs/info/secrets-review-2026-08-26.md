...
| `apps/auth-worker/src/env.ts:209` + `CREDENTIALS.md` §7 rule 7 | `TOKEN_SIGNER_KEY` **"DOES NOT EXIST YET"** / is a *deliberately unset* secret | 🔴 **Wrong.** `TOKEN_SIGNER_KEY` **IS SET** on `estate-auth`. Either it was set and the comments never caught up, or something set it unnoticed. ⚠️ **This is the impersonation-capable key** — the one where a wrong belief costs most. ✅ **CORRECTED 2026-08-26** (re-measured the same way, `wrangler secret list`): `env.ts`, `src/token-signer.ts`, `tools/estate-probes/probes/auth-worker.mjs` and `access/estate-auth.md` §3.3/§6/§7 all now say it is set and carry the custody line from §3.1 (Google-issued, GCP console, two keys valid at once). ⚠️ **Three more copies of the stale claim were found than this row named** — a comment, a probe note and a doc gotcha, each phrased as "idle"/"does not exist yet" rather than in this row's words, which is why grepping for the SECRET NAME rather than the wording is what found them |
