...
| 12 | §7.1 makes the registry's `sites` field a UI fact only — a pair that does not exist draws `n/a` and the write door says nothing about it | **the door refuses the pair too**: `feature !== '*' && site !== '*' && !f.sites.includes(site)` → 400 `feature_not_on_site`, naming the feature's label, the site, and the sites it IS on (`5714993`, W3-PLATSMALL 2026-09-05, 🔴 NOT DEPLOYED — rides the pending `estate-auth` deploy) | Row 5's lesson a third time, after row 10: `n/a` is a UI rule and a UI rule is one fetch away from being bypassed. `{feature:'cli.backfill', site:'games'}` stored cleanly until this landed and denied nobody, ever — the games catalogue has no command-line backfill, so nothing there asks about that id, exactly the switch-nobody-pressed class §3.2 refuses a typo'd id for. `*` on either side stays exempt: both wildcards resolve correctly and neither can name a pair that exists nowhere |
