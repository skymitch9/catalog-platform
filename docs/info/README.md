...
| [`estate-themes.md`](estate-themes.md) | The estate THEME SYSTEM (built 2026-08-13, canonical asset in `sites/heygabi-home/public/assets/`): three user-selectable themes — apple (authored), cyberpunk (extracted from the audiobook site), retro (extracted from the games app; ⚠️ actually muted vintage pop-art) — × light/dark on one `--et-*` token contract; the cog switcher (`hg_theme`/`hg_mode`), per-site identity defaults (owner decision), the motion vocabulary, and the honest per-site fidelity costs |
