...
> the deploy pair). ⚠️ **No other row was re-checked that day.**
> Previously **Last verified: 2026-09-05.**
>
...
>
> ### 🔴 AMENDED 2026-09-06 — two facts in this document are WRONG, and building the routes is how it was found
>
> | Where | Said | Measured 2026-09-06 |
> |---|---|---|
> | §3.1, `audit-series-aggregates.mjs` | Reads *"D1 + **HTTP** (Open Library)"* | **No HTTP at all.** It imports `@lc/core` and `scripts/lib/d1.mjs` and makes no network call |
> | §3.1, `check-cover-health.mjs` | *"**HEADs** every cover URL"* | It **GETs**. `fetch(url)` with no method; the route matches it deliberately, because image CDNs answer HEAD with 405 or no `Content-Length` |
>
> ⚠️ Both came from the header's own admission below — *"Classifications are read out of headers and call sites, not from behaviour"*. They are corrected in place rather than quietly edited away, because the SECOND one is the interesting kind of error: a route built to the inventory's word rather than the script's behaviour would have disagreed with the script on real rows, which is exactly what the shared-implementation rule in §6 exists to prevent.
>
> 🔴 A third finding, not a doc error but a code one: **`audit-series-aggregates.mjs` had no `--friend`** and had therefore never once looked at padhard. Fixed the same day; padhard measured clean for the first time.
>
...
| `check-cover-health.mjs` | HEADs every cover URL, reports broken ones and placeholder-sized files. ⚠️ **Corrected 2026-09-06: it GETs, it does not HEAD** — several image CDNs answer HEAD with a 405 or no `Content-Length`, and the route matches the script deliberately | D1 + **HTTP** | nothing (report) | unknown | ✅ **YES since 2026-09-06** — §7 #5 | **ROUTE+CRON** — ✅ **DONE** | **high** — pure HTTP + D1 read, zero disk, and a report nobody remembers to run is a report that never runs |
...
| `audit-series-aggregates.mjs` (`audit:series-aggregates`) | Standing alarm for Open Library work-level aggregates (tier 3 of the bare-series rule) | 🔴 **D1 ONLY — corrected 2026-09-06.** This column said *"D1 + HTTP (Open Library)"*; **measured, it makes no network call of any kind.** The Open Library connection is in what the alarm is ABOUT, not in how it looks — which is why it can share the cover audit's cron without competing for a subrequest budget | nothing (report) | unknown — described as "the standing alarm", which implies a cadence nothing enforces | ✅ **YES since 2026-09-06** — §7 #6 | **ROUTE+CRON** — ✅ **DONE** | medium-high — a *standing alarm* with no clock is the exact failure this ask is about |
...
| **2** | ✅ **BUILT + DEPLOYED TO BOTH 2026-09-05** — `backfill:series-volumes` → the **same** audiobook cron, one invocation, two halves. 🔴 **Shipped in SHADOW under the SAME `AUDIOBOOK_SWEEP_MODE`** — no second switch, so one flip enforces both and the ≥42-tick gate now needs zero divergences on BOTH comparisons. `/api/health` carries `detail.audiobookSweep.seriesVolumes`. ⚠️ **Rung 2 (Open Library) STAYS in the script** — one serial HTTP call per work with an OL id is a cron tick's whole subrequest budget — so the script is the more capable instrument and is never retired. ⚠️ **NO migration**: `series_volume` / `series_check` are 0003/0200 and already on both. Facts: [`…/library_catalog/docs/info/series-formats-and-audiobooks.md` §4.13](../../../bookbuddy/library_catalog/docs/info/series-formats-and-audiobooks.md) · runbook [`…/docs/access/audiobook-sweep.md`](../../../bookbuddy/library_catalog/docs/access/audiobook-sweep.md) §4a. Commits `e1e5755` (shared planner + one rendering) · `8cbdc40` (the script becomes a thin caller) · `b2c9931` (the cron half + health). Versions MAIN `6ed4a22b` / friend `c57c5173`. **Parity measured on both, script and route identical:** MAIN 139 series / 329 statements / 69 new; padhard 313 / 453 / 140. 🔴 **padhard's `series_volume` is EMPTY** — nobody was running that script on her instance, which is the size of what enforcing would land | **Same input, same fetch, same instance pair** — once the CSV fetch and the shared parser exist, this costs one function. Two audiobook-derived tables that fall out of step is a worse bug than either being stale |
| **3** | ✅ **BUILT 2026-09-05** — `prune-r2-backups.mjs` → `estate-auth` cron `41 10 * * *`, ⚠️ **shipped in SHADOW.** Facts + the enforce gate: [`../access/backup-restore.md` §3.1b](../access/backup-restore.md) | Pure R2 list+delete, no disk, no local key. **Retention that depends on somebody remembering is not retention**, and the estate's backup docs already treat retention as a promise |
| **4** | ✅ **BUILT 2026-09-05** — `tools/estate-probes/run.mjs` → `estate-auth` cron `19 * * * *`, hourly; the list moved to `lib/suite.mjs` and both runners import it. `/api/health` carries `detail.estateProbes`; ⏳ **the `/status` LINE is still open** (`../TODO.md`). Facts: `tools/estate-probes/README.md` | Pure HTTP against live hosts. This is the suite that catches an authorised-domain change *before* somebody quotes a 40-minute-old reading as current — the exact 2026-08-16 incident in the global rules |
| **5** | ✅ **BUILT + DEPLOYED TO BOTH 2026-09-06** — `check-cover-health.mjs` → library Worker, cron `47 9 * * *`. 🔴 **The cap is 250 URLs a night on a WRAPPING window** (a `LIMIT` would audit the first 250 forever and report itself clean), and `unreachable` is counted apart from `broken`. `/api/health` carries `detail.coverHealth`. Facts: [`../../../bookbuddy/library_catalog/docs/info/audit-routes.md`](../../../bookbuddy/library_catalog/docs/info/audit-routes.md) · runbook [`…/docs/access/audits.md`](../../../bookbuddy/library_catalog/docs/access/audits.md). Commits `68d320c` (shared rules) · `c519a8c` (the script becomes a thin caller) · `91405d5` (cron + runners + health) · `888ffbe` (docs). ⚠️ **The script STAYS and is the more capable instrument — it has no cap.** ⏳ No cron tick observed yet | D1 read + HTTP HEAD, nothing else. Broken covers are the estate's most visible silent rot, and both instances need it |
| **6** | ✅ **BUILT + DEPLOYED TO BOTH 2026-09-06** — `audit-series-aggregates.mjs` → the SAME cron, one invocation, two audits. `/api/health` carries `detail.seriesAggregates`. 🔴 **Two corrections measured while building it:** this row's *"Reads: D1 + HTTP (Open Library)"* is **wrong — it makes no network call at all** (which is why the two audits can share a cron: only one spends subrequests); and the script had **no `--friend`**, so the standing alarm had never once looked at padhard — fixed in `681d681`, and padhard measured clean (309 series names, 4 multi-edition works, 0 flagged) for the first time. Same commits as #5 | *"a **standing alarm** with no clock is the exact failure this ask is about"* — and it had no clock for three weeks |
