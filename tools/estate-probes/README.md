...
> `gabi_books_tools` against ~~the four names~~ **the names** in
> `GABI_BOOKS_TOOL_NAMES`, so the allowlist is asserted against what is
> DEPLOYED and not only against what is built. ⚠️ **Corrected 2026-09-05:** it
> was written as *four* on 2026-09-01 and `count_phrase` became the **fifth**
> on 2026-09-03 without the probe's array being updated, so D5 failed on every
> run for two days — see the *Gotchas* row about a stale expectation. The
> earlier `I9`–`I11` machine-read caveat is resolved: they pass.
>
...
| `discord.heygabi.ai` | `probes/discord-worker.mjs` | **LIVE — switched on 2026-09-01** (it had printed "not deployed yet (expected)" for weeks after actually deploying; see the header note). `/api/health` → 200, `service === "estate-discord"`, the `{ ok, service, configured }` envelope with config-PRESENCE booleans printed (never values), and **`D5`: `gabi_books_tools` equals the FIVE names in `GABI_BOOKS_TOOL_NAMES`** (`book_presence`, `count_phrase`, `list_book_knowledge`, `read_book_passage`, `search_book_text`) — the fourth allowlist asserted against the DEPLOY, not just the build. ⚠️ **It read "four" until 2026-09-05**, two days after `count_phrase` joined the array (`272ac67`, 2026-09-03), and the probe failed on every run in between; asserted as a SET on purpose — a length check would silently accept a swap, and a subset test would accept a sixth tool nobody decided on. Posture/readiness (`gabi_books_enabled`, `gabi_books_ready`, the caps) are PRINTED but never failed on: `GABI_BOOKS = "off"` is the documented rollback, and a suite that failed on it would fight the lever it exists to keep safe. `POST /interactions` is deliberately NOT probed — Ed25519-gated Discord machinery, covered by the worker's own tests |
...
| ⚠️ **A probe that pins a DEPLOYED allowlist goes stale the day the allowlist grows, and nothing links the two** | MEASURED 2026-09-05: `discord:D5` pinned `gabi_books_tools` to **four** names; `count_phrase` became the fifth on 2026-09-03 (`272ac67`, deployed the same day) and D5 failed on **every run for two days**. Two separate `deploys.log` entries named it as *"the PRE-EXISTING stale `discord:D5`"* and neither opened an item, which is the real cost — ⚠️ **a suite with one standing false failure is a suite people stop reading, and the next genuine failure hides behind it.** Fixed by updating the array, never by relaxing the assertion. **The rule that falls out: adding a name to any allowlist a probe pins means editing the probe in the SAME commit**, exactly like the "new endpoints get a probe" rule above |
