...
describe('⚠️ THE ELIGIBILITY ALLOWLIST — read-only names, written out, default-deny', () => {
  ...
  it('⚠️ ONE unlisted tool takes the WHOLE loop off Groq — not merely that turn', () => {
    ...
    // PROPOSES the write.
    // ⚠️ **`count_phrase` WAS held off the allowlist on 2026-09-03 and IS ON IT
    // NOW, the same day.** The original reasoning (design §4.3 of
    // `gabi-phrase-count-and-read-state.md`: *"Groq: out — tools stay on
    // Anthropic"*) was about the ANSWER it produces — a single number a person
    // will quote back. What that reasoning did not price is the PER-LOOP gate:
    // the book tools are offered as a family, so one unlisted name among them
    // took the WHOLE converse loop off Groq. Measured live at 19:12Z from
    // `wrangler tail`: `outcome:"ineligible"`,
    // `ineligible_reason:"tool_not_allowlisted"`, `blocked_tools:["count_phrase"]`,
    // `tools_offered: 11` — every converse loop, sunk to Haiku, by one name.
    // It is read-only (GET; counts + chapter anchors + ≤3 quotes ≤400 chars),
    // a strict subset of what `search_book_text` — already allowlisted — sends.
    //
    // ⚠️ So this assertion is the inverse of what it was: an EMPTY blocked list
    // over every offered tool is what proves the converse loop is eligible.
    assert.deepEqual(groqBlockedTools(everything), [], 'the allowlist has drifted');
