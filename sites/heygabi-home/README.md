...
| `public/assets/apex-admin-link.js` | ⚠️ **Missing from this table until 2026-09-05 — a finding, not a tidy-up.** The front door's gated Admin card: listens for `estate-search:auth` on `#find-search`, probes `GET /api/estate/me` once per signed-in uid, and reveals the card by tier (approver → all three links; devops → Status only). **The estate's canonical fail-hidden seam:** any failure leaves the card hidden, because "the links are conveniences, the pages exist without them and enforce themselves" |
| `public/assets/apex-request-catalog.js` | The **"+" on the Books and Games cards** (owner ask 2026-09-05; design `docs/info/request-a-catalog-design.md` §4) — a signed-in, estate-`approved` member asks for a catalog of their own at `<them>.heygabi.ai`. Same auth seam and same fail-hidden posture as the row above, per kind: signed out or unapproved or probe-failed → nothing renders; a `pending`/`accepted` request → a "Requested — pending review" pill with a two-tap Withdraw; a `live` catalog of that kind → gone for good. 🔴 **It files a request; it does not create a catalog.** Its styles live in `index.html`'s own `<style>`, following the row above (which has no stylesheet either). ⚠️ Deploying it needs the auth Worker's CORS mounts committed first — `docs/info/worktree-deploys.md` §5 |
